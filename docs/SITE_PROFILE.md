...
7. **RULE — On staging, rendered Yoast SEO output must never be used as evidence of current
   SEO metadata. Raw postmeta is the source of truth for validation.** Reason: Yoast does not
   rebuild indexables in non-production environments (`WP_ENVIRONMENT_TYPE`), so staging pages
   render whatever indexable rows came with the production clone — postmeta written since (by
   the connector, wp-admin, or anything else) is stored correctly but never emitted.
   (Discovered 2026-07-20, Test 4 — don't spend two hours staring at page source.)
   Corollaries: (a) apply-time RENDER checks are only meaningful on production, or with the
   optional 3-line mu-plugin override recorded in VALIDATION.md; (b) the same test found and
   fixed connector v0.1.2's bilingual-loss bug — qTranslate language-filters postmeta READS
   in REST contexts, so raw `$wpdb` reads are required when copying multilingual values.
