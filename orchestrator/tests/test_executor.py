...
import tc_growth.core.actions as _actions
...
# The Scenario-B foundation registry contains only two READ_ONLY diagnostics, so there is no
# write/ALWAYS_ASK operation to exercise the executor's guardrails against. Inject a synthetic
# one for those tests — it tests the EXECUTOR (unchanged), not the catalogue.
_SYNTH_WRITE = Operation(
    id="synth_write", name="Synthetic write op (test-only)",
    category=Category.EXECUTION, min_phase=Phase.CONTROLLED_EXECUTION,
    environments=("staging",), approval=Approval.ALWAYS_ASK,
    command="noop", allowed_args=(),
    rollback_description="n/a (test)", verification_description="test",
)


def _inject_synth(monkeypatch):
    monkeypatch.setattr(_actions, "OPERATIONS", _actions.OPERATIONS + (_SYNTH_WRITE,))
...
def test_preview_reports_a_write_op_as_not_runnable_in_read_only_phase(monkeypatch):
    _inject_synth(monkeypatch)
    prev = _exec().preview("synth_write")
...
def test_preview_does_not_prompt_for_confirmation(monkeypatch):
    ...
    _exec(phase=Phase.CONTROLLED_EXECUTION, confirm=confirm).preview("synth_write")
    assert called["n"] == 0
...
def test_operation_below_current_phase_is_blocked(monkeypatch):
    # synth_write needs CONTROLLED_EXECUTION; a READ_ONLY executor must refuse it.
    _inject_synth(monkeypatch)
    res = _exec().execute("synth_write")
    assert res.execution_status == "blocked"
    assert res.block_reason == "phase"


def test_wrong_environment_is_blocked(monkeypatch):
    # synth_write may target staging only.
    _inject_synth(monkeypatch)
    res = _exec(phase=Phase.CONTROLLED_EXECUTION, environment="production").execute("synth_write")
    assert res.execution_status == "blocked"
...
def test_always_ask_op_without_confirmation_is_blocked(monkeypatch):
    ...
    res = Executor(phase=Phase.CONTROLLED_EXECUTION, environment="staging",
                   confirm=None, store_factory=lambda: None).execute("synth_write")
    assert res.execution_status == "blocked"
