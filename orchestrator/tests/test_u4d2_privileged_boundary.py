...
def staged_release(target, sha: str = SHA) -> Path:
    """A release directory shaped like the real one, for verbs that need one."""
    release = Path(target.target.releases_dir) / sha
    scripts = release / "orchestrator" / "scripts"
    scripts.mkdir(parents=True, exist_ok=True)
    shutil.copy2(Path(target.target.privileged_prefix) / "wp-integrity-scan.sh",
                 scripts / "wp-integrity-scan.sh")
    return release
...
def test_root_installs_the_inspector_from_its_own_copy_not_from_the_release(target):
    ...
    proc = run_verb(target, "apply", release.name)
    assert proc.returncode == 2
...
def test_rollback_restores_from_root_owned_state(target):
    ...
    assert run_verb(target, "apply", release.name).returncode in (0, 3)
