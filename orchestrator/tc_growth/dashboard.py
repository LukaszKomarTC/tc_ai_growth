...
.badge.open,.badge.monitoring{background:#fff3cd}.badge.resolved,.badge.closed{background:#d4edda}
...
def render_overview(s: Store) -> str:
    ...
    dec_rows = "".join(
        f"<tr><td>{_e(d.made_at)}</td><td>{_e(d.status)}</td><td>{_e(d.made_by)}</td>"
        f"<td>{_e(d.title)}</td><td>{_e(d.case_id and f'case #{d.case_id}')}</td></tr>"
        for d in decisions
    ) or "<tr><td colspan='5' class='muted'>no decisions logged</td></tr>"
    ...
    body = (
        ...
        "<h2>Decision log</h2><table><tr><th>made</th><th>status</th><th>by</th><th>title</th>"
...
def render_case(s: Store, key: str) -> str | None:
    ...
    dec_html = "".join(
        f"<tr><td>{_e(d.made_at)}</td><td>{_e(d.status)}</td><td>{_e(d.made_by)}</td><td>{_e(d.title)}</td></tr>"
        for d in decisions
    ...
    body = (
        ...
        f"<h2>Linked decisions</h2><table><tr><th>made</th><th>status</th><th>by</th><th>title</th></tr>{dec_html}</table>"
    )
